...
public class XRecyclerViewHeader extends LinearLayout {
    ...
    public int mMeasuredHeight = 0;//布局的原始高度，用于做状态改变的标志
...
//        SPUtils.getinstants(mContext).insert("REFRESHCOMPLATE",refTime);
        smoothScrollTo(0);
        setState(STATE_NORMAL);
    }

    public void showTime() {
        //注释的是时间文本  有需要可以去掉  也是在布局里改可见属性aa
//        mHeaderTimeView.setText(friendlyTime(SPUtils.getinstants(mContext).get("REFRESHCOMPLATE",0L)));
    }
    ...
    public static String friendlyTime(long time) {
        Log.e("TAG", "time == " + time + "  System.currentTimeMillis() == " + System.currentTimeMillis());
        if(time == 0) {
            return "刚刚";
        }
        //获取time距离当前的秒数

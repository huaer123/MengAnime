...
import static com.recyclerviewpull.XRecyclerViewHeader.STATE_REFRESHING;

/**
 * Created by zwx on 2016/12/13.
 * Explain:
 */

public class XpulltorefereshiRecyclerView extends RecyclerView {
    ...
    public XpulltorefereshiRecyclerView(Context context) {
        this(context, null);
    }

    public XpulltorefereshiRecyclerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public XpulltorefereshiRecyclerView(Context context, AttributeSet attrs, int defStyle) {
...
//    private boolean isAnimoto = false;
    public void setLoadingMoreEnabledAnimoto(boolean enabled) {//设置显示动画
//        isAnimoto = enabled;
    }
    ...
    public void onScrollStateChanged(int state) {
        ...
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadingListener != null && loadingMoreEnabled) {
            ...
            if (layoutManager.getChildCount() > 0
                    ...
                    && mHeader.getState() < STATE_REFRESHING) {
       ...
       /* //设置滑倒动画的时候 有动画
        if (state == RecyclerView.SCROLL_STATE_IDLE && mLoadingListener != null && isAnimoto) {
            LayoutManager layoutManager = getLayoutManager();
            int lastVisibleItemPosition;  //最后可见的Item的position的值
...
//        String diffDateInfo = DateUtils.getDiffDateInfo(String.valueOf(System.currentTimeMillis()));
//        Log.e("TAG", " diffDateInfo == " + diffDateInfo + "   aaa == " + DateUtils.getCurrentaDate());
        if (mLastY == -1) {
            mLastY = e.getRawY();
        }
        switch (e.getAction()) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (isOnTop() && pullRefreshEnabled) {
                    ...
                    if (mHeader.getVisiableHeight() > 0 && mHeader.getState() < STATE_REFRESHING) {
...
//                        mHeader.setState(mHeader.STATE_REFRESHING);aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa
                        return false;
                    }
                }
                break;
